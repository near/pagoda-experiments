...
export async function decryptWithPrivateKey(encryptedData: string, privateKey: any): Promise<string> {
  const encryptedDataArrayBuffer = Uint8Array.from(atob(encryptedData), (c) => c.charCodeAt(0)).buffer;
